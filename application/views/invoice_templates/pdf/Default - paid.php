...
<head>
    ...
    <link rel="stylesheet"
          href="<?php echo base_url(); ?>assets/<?php echo get_setting('system_theme', 'billite'); ?>/css/templates.css">
    ...
    <table class="item-table">
        ...
        foreach ($items as $item) { ?>
            <tr>
                ...
                <td class="text-right">
                    <?php
                    if($invoice->currency == "USD"){
                        echo client_currency($item->item_price); 
                    }else{
                ...
                <?php if ($show_item_discounts) : ?>
                    <td class="text-right">
                        <?php 
                         if($invoice->currency == "USD"){
                            echo client_currency($item->item_discount); 
                        }else{
                ...
                <td class="text-right">
                    <?php
                        if($invoice->currency == "USD"){
                        echo client_currency($item->item_total); 
                    }else{
        ...
        <?php if ($invoice->invoice_item_tax_total > 0) { ?>
            <tr>
                ...
                <td class="text-right">
                    <?php
                    if($invoice->currency == "USD"){
                        echo client_currency($invoice->invoice_item_tax_total); 
                    }else{
                        echo format_currency($invoice->invoice_item_tax_total); 
                    }
                    ?>
                </td>
            </tr>
        ...
        <?php foreach ($invoice_tax_rates as $invoice_tax_rate) : ?>
            <tr>
                ...
                <td class="text-right">
                    <?php
                    if($invoice->currency == "USD"){
                        echo client_currency($invoice_tax_rate->invoice_tax_rate_amount); 
                    }else{
                        echo format_currency($invoice_tax_rate->invoice_tax_rate_amount); 
                    }
                    ?>
                </td>
            </tr>
        ...
        <?php if ($invoice->invoice_discount_amount != '0.00') : ?>
            <tr>
                ...
                <td class="text-right">
                    <?php
                    if($invoice->currency == "USD"){
                        echo client_currency($invoice->invoice_discount_amount); 
                    }else{
        ...
        <tr>
            ...
            <td class="text-right">
                <?php
                if($invoice->currency == "USD"){
                    echo client_currency($invoice->invoice_paid); 
                }else{
