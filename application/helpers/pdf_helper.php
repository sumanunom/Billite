...
function generate_invoice_pdf($invoice_id, $stream = true, $invoice_template = null, $is_guest = null)
{
    ...
    // $invoice_item_tax_rates = $CI->mdl_invoice_tax_rates->item_tax_rate($invoice_id);
    // print_r( $invoice_item_tax_rates);
    // die();
...
function generate_quote_pdf($quote_id, $stream = true, $quote_template = null)
{
    ...
    if (!$quote_template) {
     ...
     $quote_notes = $CI->mdl_settings->setting('default_quote_notes');
    ...
    $data = array(
        ...
        'quote_item_tax_rates' => $CI->mdl_quote_tax_rates->item_tax_rate($quote_id),
        'items' => $items,
        'output_type' => 'pdf',
        'show_item_discounts' => $show_item_discounts,
        'custom_fields' => $custom_fields,
