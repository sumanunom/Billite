...
class Mdl_Invoice_Amounts extends CI_Model
{
    ...
    public function calculate($invoice_id)
    {
        ...
        // $query = $this->db->query("
        //   SELECT SUM(payment_amount) AS invoice_paid, SUM(tds_deducted) AS tds, SUM(bank_charges) AS bank
        //   FROM ip_payments
        //   WHERE invoice_id = " . $this->db->escape($invoice_id)
        // );
        // $invoice_paid_amount = $invoice_paid+$tds+$bank;

           $query = $this->db->query("
          SELECT (SUM(payment_amount)+SUM(tds_deducted)+SUM(bank_charges)) AS invoice_paid
          FROM ip_payments
          WHERE invoice_id = " . $this->db->escape($invoice_id)
        );
