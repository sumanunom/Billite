...
<form method="post" class="form-horizontal">
    ...
    <div id="content">
                        ...
                        <?php foreach ($open_invoices as $invoice) { ?>
                            <option value="<?php echo $invoice->invoice_id; ?>"
                                ...
                                if($invoice->currency == "USD") {
                                    echo $invoice->invoice_number . ' - ' . format_client($invoice) . ' - ' . client_currency($invoice->invoice_balance);
                                }else{
                                    echo $invoice->invoice_number . ' - ' . format_client($invoice) . ' - ' . format_currency($invoice->invoice_balance);
                                }
                                ?>
                            </option>
                        <?php } ?>
                    <?php } else { ?>
                        <option value="<?php echo $payment->invoice_id; ?>">
                            ...
                            if($payment->currency == "USD") {
                                echo $payment->invoice_number . ' - ' . format_client($payment) . ' - ' . client_currency($payment->invoice_balance); 
                            }else{
        ...
        <div class="form-group">
          ...
          <div class="form-group">
            <div class="col-xs-12 col-sm-2 text-right text-left-xs">
                <label for="tds_deducted" class="control-label"><?php _trans('tds_deducted'); ?></label>
            </div>
            <div class="col-xs-12 col-sm-6">
                <input type="text" name="tds_deducted" id="tds_deducted" class="form-control"
                value="<?php echo format_amount($this->mdl_payments->form_value('tds_deducted')); ?>">
            </div>
        </div>  

        <div class="form-group">
            <div class="col-xs-12 col-sm-2 text-right text-left-xs">
            <label for="bank_charges" class="control-label"><?php _trans('bank_charges'); ?></label>
            </div>
            <div class="col-xs-12 col-sm-6">
                <input type="text" name="bank_charges" id="bank_charges" class="form-control"
                    value="<?php echo format_amount($this->mdl_payments->form_value('bank_charges')); ?>">
            </div>
        </div>        
