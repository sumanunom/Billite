...
<script>
    ...
    $(function () {
        ...
        // Get the latest version from the Billite IDS
        $.ajax({
            'url': 'https://ids.billite.in/updatecheck?cv=' + ip_version,
            'dataType': 'json',
            success: function(data) {
                ...
                setTimeout(function() {
                    if (update(ip_version, updatecheck)) {
                        $('#updatecheck-updates-available').attr("href", "https://www.billite.in/downloads")
            ...
            error: function(data) {
                $.ajax({
                    'url': 'https://ids.billite.org/updatecheck?cv=' + ip_version,
                    'dataType': 'json',
                    success: function(data) {
                        <?php echo(IP_DEBUG ? 'console.log(data);' : ''); ?>
                        var updatecheck = data.current_version;
                        ...
                        setTimeout(function() {
                            if (update(ip_version, updatecheck)) {
                                $('#updatecheck-updates-available').attr("href", "https://www.billite.org/downloads")
                                $('#updatecheck-loading').addClass('hidden');
                                $('#updatecheck-updates-available').removeClass('hidden');
                            }
                            else {
        ...
        // Get the latest news
        $.ajax({
            'url': 'https://ids.billite.in/get_news',
            'dataType': 'json',
            'success': function(data) {
                <?php echo(IP_DEBUG ? 'console.log(data);' : ''); ?>
                setTimeout(function() {
            ...
            'error': function(data) {
                $.ajax({
                    'url': 'https://ids.billite.org/get_news',
                    'dataType': 'json',
                    'success': function(data) {
                        <?php echo(IP_DEBUG ? 'console.log(data);' : ''); ?>
                        setTimeout(function() {
